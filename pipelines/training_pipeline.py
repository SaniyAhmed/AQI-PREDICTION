...
def get_forecast_features(trained_columns, latest_actuals):
    # Fetching real-time weather + air quality forecast
    ...
    prep = pd.DataFrame({
        ...
        'pm25': df_f['pm2_5'].astype('float64'),
...
def run_pipeline():
    ...
    # Identify the "Seed" (150) and the Karachi Mean
    ...
    karachi_mean_aqi = full_df['aqi'].mean()
    ...
    # Professional XGBoost tuning to prevent over-reliance on a single feature
    model = XGBRegressor(
        n_estimators=200, 
        learning_rate=0.03, # Slower learning = better trend capture
        max_depth=4, 
        reg_lambda=10, # Penalty to prevent "Lag Addiction"
        random_state=42
    )
    model.fit(X_train_s, y_train.loc[X_train.dropna().index].values.ravel())

    # --- DYNAMIC RECURSIVE FORECAST WITH ALPHA-DECAY ---
    X_f_base, times = get_forecast_features(feature_names, latest_actuals)
    
    predictions = []
    current_lag_aqi = current_aqi

    print(f"📍 STARTING POINT: {current_aqi}")
    ...
    for i in range(len(X_f_base)):
        ...
        # We blend the prediction with a "Return to Mean" factor.
        # This prevents the flat-line at 150 by pulling the value 
        # towards the historical average by 5% every hour.
        decay_strength = 0.05 
        pred = (pred * (1 - decay_strength)) + (karachi_mean_aqi * decay_strength)
    ...
    print("\n🔮 PROFESSIONAL TREND FORECAST (Next 10 Hours):")
    print("-" * 45)
    ...
    print("\n✅ Success: Predictions are now dynamic.")
